...
async function submitAnswer(req, res) {
  const { questionId } = req.body; // Note: Changed to body from params
  const { answer } = req.body;

  if (!answer || !questionId) {
    return res.status(StatusCodes.BAD_REQUEST).json({
      success: false,
      message: "Invalid input. Please provide a valid answer and question ID.",
    });
  }
  ...
  try {
    ...
    await dbConnection.query(
      "INSERT INTO answers (userId, question_id, answer) VALUES (?, ?, ?)",
...
module.exports = { getAnswersForQuestion, submitAnswer };
